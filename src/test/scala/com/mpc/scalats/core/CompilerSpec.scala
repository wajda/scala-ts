...
class CompilerSpec
  ...
  it should "compile scala model" in {
    ...
    val decls = Compiler.compile(Seq(
      ScalaModel.ObjectEntity(
        ...
        Seq(
          ...
          ScalaModel.EntityMember("Foo", ScalaModel.StructRef, ScalaModel.StructValue(
            ScalaModel.EntityMember("z", ScalaModel.SeqRef(ScalaModel.IntRef), ScalaModel.SimpleValue(Seq(1, 2, 3), ScalaModel.SeqRef(ScalaModel.IntRef))),
    ...
    decls should contain theSameElementsAs Seq(
      TypeScriptModel.ConstantDeclaration(
        ...
        TypeScriptModel.ObjectValue(Seq(
          (TypeScriptModel.Member("bar", TypeScriptModel.ObjectRef), TypeScriptModel.ObjectValue(Seq(
          ...
          (TypeScriptModel.Member("Foo", TypeScriptModel.ObjectRef), TypeScriptModel.ObjectValue(Seq(
            (TypeScriptModel.Member("z", TypeScriptModel.ArrayRef(TypeScriptModel.NumberRef)), TypeScriptModel.PrimitiveValue(Seq(1, 2, 3), TypeScriptModel.ArrayRef(TypeScriptModel.NumberRef))),
